  ...
  return (
    <main className="min-h-screen bg-[#f5f7fc] px-5 py-14 text-slate-900">
      <div className="mx-auto w-full max-w-xl">
        ...
        <p className="mt-4 text-sm leading-7 text-slate-600">승인된 운영자만 AI 상담 품질·비용 현황을 볼 수 있습니다.</p>
      </div>
    </main>
  );
}
    ...
    return (
      <main className="min-h-screen bg-[#f5f7fc] px-5 py-10 text-slate-900 sm:px-8 sm:py-14">
        <div className="mx-auto w-full max-w-6xl">
          <Link href="/admin" className="text-sm font-semibold text-slate-600 underline underline-offset-4">← 운영 콘솔</Link>
    ...
    <main className="min-h-screen bg-[#f5f7fc] px-5 py-10 text-slate-900 sm:px-8 sm:py-14">
      <div className="mx-auto w-full max-w-6xl">
        <header className="border-b border-slate-200 pb-6">
          <Link href="/admin" className="text-sm font-semibold text-slate-600 underline underline-offset-4">← 운영 콘솔</Link>
          <p className="mt-7 text-xs font-semibold tracking-[0.2em] text-slate-500">AI CONSULTING · PHASE 12A</p>
        ...
        <section className="mt-8 border-y border-slate-200 py-6">
          <div className="grid gap-6 lg:grid-cols-2">
            <div>
              <p className="text-xs font-semibold tracking-[0.16em] text-slate-500">SCOPE ROUTING</p>
              <h2 className="mt-2 text-xl font-bold">최근 질문 분류</h2>
              <p className="mt-2 text-sm text-slate-600">최근 사용자 질문 최대 200건 기준입니다.</p>
            ...
            <div>
              <p className="text-xs font-semibold tracking-[0.16em] text-slate-500">COST TELEMETRY</p>
              ...
              <div className="mt-4 border border-slate-200 bg-white p-4">
                ...
                <p className="mt-1 text-xs text-slate-500">가격을 알 수 없는 모델/텔레메트리 {number(report.unknownPriceCount)}건</p>
                {modelEntries.length > 0 ? (
                  <ul className="mt-3 space-y-1 text-sm text-slate-700">
                    {modelEntries.map(([model, count]) => <li key={model}>{model} · {number(count)}건</li>)}
                  </ul>
                ) : <p className="mt-3 text-sm text-slate-500">아직 완료된 AI 상담이 없습니다.</p>}
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="mt-8">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="text-xs font-semibold tracking-[0.16em] text-slate-500">RECENT SAMPLES</p>
          ...
          {report.samples.length === 0 ? (
            <div className="mt-4 border border-dashed border-slate-300 bg-white p-8 text-center text-sm leading-7 text-slate-500">
              아직 완료된 AI 상담 답변이 없습니다. 실제 상담이 발생하면 이 화면에서 토큰·원가와 구조 검사를 확인할 수 있습니다.
            </div>
          ) : (
            <div className="mt-4 space-y-3">
              {report.samples.map((sample) => (
                <article key={sample.assistantMessageId} className="rounded-2xl border border-[#dce1ef] bg-white p-5 shadow-sm">
                  ...
                  <div className="mt-4 grid gap-4 lg:grid-cols-2">
                    <div>
                      ...
                      <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-800">{sample.questionPreview || "질문 연결 정보 없음"}</p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-slate-500">답변 미리보기</p>
                      <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{sample.answerPreview}</p>
                    </div>
                  </div>
