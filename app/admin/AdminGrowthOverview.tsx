...
export default function AdminGrowthOverview({
  ...
  return (
    <section className="border-b border-slate-200 pb-10" aria-labelledby="growth-overview-heading">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="border border-slate-200 bg-white px-4 py-3 text-xs leading-5 text-slate-600">
      ...
      <div className="mt-7 rounded-2xl border border-[#dce1ef] bg-white p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">같은 날의 단계별 운영량 비율입니다. 코호트 전환율이 아니라 당일 퍼널 상태를 빠르게 보는 지표입니다.</p>
          </div>
        </div>
        <div className="mt-5 grid gap-2 md:grid-cols-5">
          {funnel.map((item, index) => (
            <div key={item.label} className="relative border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs font-semibold text-slate-500">{index + 1}. {item.label}</p>
              <p className="mt-2 text-xl font-bold">{number(item.value)}</p>
              <p className="mt-2 text-xs text-slate-600">이전 단계 대비 {item.rate}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-[#dce1ef] bg-white p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">일별 방문·무료분석·가입·인증·구매·순매출</p>
            </div>
            <div className="flex gap-2">
              {[7, 30].map((days) => (
                <button
                  ...
                  className={`border px-3 py-2 text-xs font-semibold ${trendDays === days ? "border-slate-900 bg-slate-900 text-white" : "border-slate-300 bg-white text-slate-700"}`}
                >
                  최근 {days}일
                </button>
              ))}
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-[#dce1ef] bg-white p-5">
            ...
            <div className="mt-4 space-y-3 text-sm">
              {[
                ...
                return (
                  <div key={label as string} className="border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                    ...
                    <p className="mt-1 text-xs text-slate-500">결제 {won(value.grossRevenueKrw)} · 환불 {won(value.refundAmountKrw)} · 성공 {number(value.paidOrders)}건</p>
                  </div>
                );
              })}
              <div className="border-t border-slate-200 pt-3">
                ...
                <p className="mt-1 text-xs text-slate-500">결제 {won(totals.grossRevenueKrw)} · 환불 {won(totals.refundAmountKrw)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-[#dce1ef] bg-white p-5">
            ...
            <dl className="mt-4 space-y-2 text-sm">
              <div className="flex justify-between"><dt className="text-slate-500">고유 방문 브라우저</dt><dd className="font-semibold">{number(totals.visitors)}명</dd></div>
              <div className="flex justify-between"><dt className="text-slate-500">무료분석 완료</dt><dd className="font-semibold">{number(totals.freeAnalysisCompleted)}건</dd></div>
              <div className="flex justify-between"><dt className="text-slate-500">추적 이후 가입</dt><dd className="font-semibold">{number(totals.signups)}명</dd></div>
              <div className="flex justify-between"><dt className="text-slate-500">현재 활성 계정</dt><dd className="font-semibold">{number(totals.activeAccounts)}명</dd></div>
              <div className="flex justify-between"><dt className="text-slate-500">현재 성인인증 계정</dt><dd className="font-semibold">{number(totals.currentVerifiedAdults)}명</dd></div>
              <div className="flex justify-between"><dt className="text-slate-500">누적 구매 고객</dt><dd className="font-semibold">{number(totals.payingCustomers)}명</dd></div>
              <div className="flex justify-between"><dt className="text-slate-500">월 운세 갱신</dt><dd className="font-semibold">{number(totals.monthlyRefreshes)}건</dd></div>
            </dl>
          </div>
        </div>
      </div>
        ...
        <div className="mt-7 rounded-2xl border border-[#dce1ef] bg-white p-5">
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-[640px] w-full text-sm">
              <thead><tr className="border-b border-slate-200 text-left text-xs text-slate-500"><th className="py-2 pr-3">상품</th><th className="py-2 pr-3">결제</th><th className="py-2 pr-3 text-right">총 결제액</th><th className="py-2 pr-3 text-right">환불</th><th className="py-2 text-right">순매출</th></tr></thead>
              <tbody>
