...
assert(result.includes("providedResult ? \"/recommendations\""), "Guest result opens personalized recommendations without forcing login first");
...
assert(shell.includes('item.activeHref !== "/saju"') && shell.includes('item.activeHref !== "/recommendations"') && shell.includes('item.activeHref !== "/deep-analysis"'), "Guest lock treatment excludes free, recommendation, and public deep-analysis destinations");
...
assert(shell.includes('const recommendationHref = isGuest === true') && shell.includes('? "/recommendations"'), "confirmed Guests and unresolved auth both keep recommendation navigation on the public recommendation route");
assert(shell.includes('if (item.href === "/recommendations") return "/recommendations";') && shell.includes("While auth is unresolved, use canonical destinations instead of manufacturing a login URL"), "recommendations stay public while other member-only destinations retain their auth boundary");
