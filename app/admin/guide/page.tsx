    ...
    return (
      <main className="min-h-screen bg-[#f5f7fc] px-5 py-14 text-slate-900">
        <div className="mx-auto w-full max-w-xl">
          ...
          <p className="mt-4 text-sm leading-7 text-slate-600">승인된 운영자만 운영 가이드를 볼 수 있습니다.</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#f5f7fc] px-5 py-10 text-slate-900 sm:px-8 sm:py-14">
      <div className="mx-auto w-full max-w-5xl">
        <Link href="/admin" className="text-sm font-semibold text-slate-600 underline underline-offset-4">← 운영 대시보드</Link>
        <header className="mt-6 border-b border-slate-200 pb-6">
          <p className="text-xs font-semibold tracking-[0.2em] text-slate-500">OWNER RUNBOOK</p>
        ...
        <section className="mt-6 rounded-2xl border border-[#dce1ef] bg-white p-5 shadow-sm">
        ...
        <section className="mt-8 space-y-4">
          {cases.map((item) => (
            <article key={item.title} className="rounded-2xl border border-[#dce1ef] bg-white p-5 shadow-sm">
              <h2 className="text-lg font-bold">{item.title}</h2>
              <dl className="mt-4 grid gap-3 text-sm leading-6 md:grid-cols-3">
                <div><dt className="text-xs font-semibold text-slate-500">언제</dt><dd className="mt-1 text-slate-800">{item.when}</dd></div>
                <div><dt className="text-xs font-semibold text-slate-500">무엇을 어떻게</dt><dd className="mt-1 text-slate-800">{item.do}</dd></div>
                <div><dt className="text-xs font-semibold text-slate-500">왜</dt><dd className="mt-1 text-slate-800">{item.why}</dd></div>
              </dl>
            </article>
          ))}
        </section>
