    ...
    <section>
      <header className="border-b border-slate-200 pb-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold tracking-[0.2em] text-slate-500">SUPPORT QUEUE</p>
            <h1 className="mt-3 text-3xl font-bold">고객지원 요청</h1>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">자동 해결 안내로 해결되지 않은 문의만 들어옵니다. 결제·환불·이용권·리포트 상태는 이 화면에서 직접 바꾸지 않고 기존 운영 조회로 확인합니다.</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm"><span className="text-slate-500">현재 목록 </span><strong>{activeCount}건</strong></div>
        </div>
      </header>

      <div className="mt-6 flex flex-wrap items-center gap-2">
        {filterOptions.map((option) => (
          <button key={option.value} type="button" onClick={() => { setFilter(option.value); void load(option.value); }} className={`rounded-full border px-3.5 py-2 text-sm font-semibold ${filter === option.value ? "border-slate-900 bg-slate-900 text-white" : "border-slate-300 bg-white text-slate-700"}`}>{option.label}</button>
        ))}
        <button type="button" onClick={() => void load()} disabled={loading} className="ml-auto rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 disabled:opacity-50">{loading ? "조회 중" : "새로고침"}</button>
      </div>

      {message ? <p role="status" className="mt-4 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">{message}</p> : null}

      {requests.length === 0 ? <p className="mt-6 rounded-xl border border-slate-200 bg-white p-6 text-sm text-slate-500">현재 표시할 문의가 없습니다.</p> : (
        <div className="mt-6 space-y-4">
          {requests.map((item) => (
            <article key={item.id} className="rounded-2xl rounded-2xl border border-[#dce1ef] bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold text-slate-500">{SUPPORT_CATEGORY_LABELS[item.category]}</p>
                  <p className="mt-1 break-all text-sm font-bold text-slate-900">{item.contactEmail}</p>
                </div>
                <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">{SUPPORT_STATUS_LABELS[item.status]}</span>
              </div>
              <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-700">{item.message}</p>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <Link href="/admin" className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700">운영 대시보드에서 고객/주문 조회</Link>
              </div>
              {item.operatorResponse ? (
                <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                  <p className="text-xs font-semibold text-emerald-700">현재 고객에게 표시되는 답변</p>
                  <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-800">{item.operatorResponse}</p>
                </div>
              ) : null}

              <form onSubmit={(event) => void save(event, item.id)} className="mt-5 border-t border-slate-100 pt-5">
                <div className="grid gap-3 sm:grid-cols-[180px_1fr]">
                  <label className="text-sm font-semibold">상태
                    <select value={statuses[item.id] ?? (item.status === "OPEN" ? "IN_REVIEW" : item.status)} onChange={(event) => setStatuses((current) => ({ ...current, [item.id]: event.target.value as SupportRequestStatus }))} className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 font-normal">
                  ...
                  <label className="text-sm font-semibold">고객에게 표시할 답변
                    <textarea value={responses[item.id] ?? item.operatorResponse ?? ""} onChange={(event) => setResponses((current) => ({ ...current, [item.id]: event.target.value }))} maxLength={1500} rows={4} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal leading-6" placeholder="확인 결과와 고객이 다음에 해야 할 행동만 적어 주세요. 내부 로그·비밀값·사주 상세는 적지 않습니다." />
                  </label>
                </div>
                <div className="mt-3 flex justify-end">
                  <button type="submit" disabled={savingId !== null} className="rounded-lg bg-[#171a3d] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#242957] disabled:bg-slate-400">{savingId === item.id ? "저장 중..." : "상태/답변 저장"}</button>
                </div>
              </form>
            </article>
          ))}
