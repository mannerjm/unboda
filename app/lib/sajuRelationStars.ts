...
function canonicalPair(first: string, second: string): string {
  const firstIndex = branchOrder.indexOf(first as (typeof branchOrder)[number]);
  const secondIndex = branchOrder.indexOf(second as (typeof branchOrder)[number]);
...
function pairMatches(pairSet: Set<string>, first: string, second: string): boolean {
  ...
  return pairSet.has(direct) || pairSet.has(reverse);
}
...
export function calculateSajuRelationStars(value: RelationCarrier): SajuRelationStar[] {
    ...
    for (let right = left + 1; right < pillars.length; right += 1) {
      ...
      if (pairMatches(wonjinPairs, first.branch, second.branch)) {
        results.push({
          ...
          basis: canonicalPair(first.branch, second.branch),
        });
      }
  ...
  if (voidBranches.length === 2) {
    const voidPositions = pillars
      .filter((pillar) => voidBranches.includes(pillar.branch))
      .map((pillar) => pillar.position);
    const matchedBranches = pillars
      .filter((pillar) => voidBranches.includes(pillar.branch))
      .map((pillar) => pillar.branch);

    if (voidPositions.length > 0) {
      results.push({
        name: "공망",
        kind: "void",
        positions: voidPositions,
