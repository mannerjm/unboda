    ...
    <article className="mt-8 overflow-hidden rounded-[32px] border border-[#dfe3ef] bg-[#f9faff] shadow-[0_22px_60px_rgba(32,38,72,0.09)]">
      <header className="bg-[linear-gradient(135deg,#f1f0ff_0%,#fbfcff_58%,#eef1fb_100%)] px-6 py-8 sm:px-9 sm:py-10">
        ...
        {!meta.familyMemberBirthTimeKnown ? (
          <p className="mt-5 rounded-2xl bg-white/65 px-4 py-3 text-xs leading-6 text-slate-600 ring-1 ring-[#dce1ef]">상대 가족의 출생시간을 모르는 경우 확인 가능한 생년월일 범위와 구매 연도 흐름을 중심으로 분석합니다.</p>
        ) : null}
      </header>
      ...
      <section className="border-y border-[#d8d3ff] bg-[#f3f1ff] px-6 py-8 sm:px-9 sm:py-10">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">가까운 가족이라도 같은 행동이 서로에게 다르게 느껴질 수 있어 두 방향을 따로 확인합니다.</p>
      ...
      <p className="border-t border-[#dce1ef] bg-white px-6 py-5 text-center text-xs leading-6 text-slate-500">이 리포트의 연도 흐름은 구매 당시 {meta.evaluationYear}년판으로 고정되어 보관됩니다. 가족 궁합은 명리 구조와 해당 연도 흐름을 해석한 참고 콘텐츠이며 가족 관계의 결과를 확정하거나 한쪽의 잘잘못을 판단하지 않습니다.</p>
    </article>
  );
}
