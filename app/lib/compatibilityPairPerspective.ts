  ...
  if (item.level === "supportive") {
    return {
      ...
      summary: supportElement
        ? `${supportElement} 기운을 중심으로 보완 작용이 더 크게 나타납니다. ${receiver}의 선택과 움직임을 넓혀 주는 방향으로 연결되기 쉬운 편입니다.`
  ...
  if (item.level === "burdensome") {
    return {
      ...
      summary: burdenElement
        ? `${burdenElement} 기운에서 부담 작용이 상대적으로 더 두드러집니다. 좋은 의도라도 ${receiver}가 받아들이는 속도와 방식을 확인하며 조절하는 편이 좋습니다.`
  ...
  if (item.level === "mixed") {
    if (supportElement && burdenElement) {
      return {
        ...
        summary: `보완 쪽에서는 ${supportElement} 기운, 부담 쪽에서는 ${burdenElement} 기운이 상대적으로 두드러집니다. 같은 행동도 상황에 따라 도움이 되거나 압박으로 느껴질 수 있어 속도와 방식을 맞추는 것이 중요합니다.`,
      };
    }
    return {
      headline: `${receiver}에게 도움과 부담이 함께 작동해요`,
...
function buildSignals(
  ...
  if (supportElement) {
    signals.push(`${supportElement} 기운은 ${receiver}에게 보완적으로 작용하는 쪽이 더 큽니다.`);
  }
  if (burdenElement) {
    signals.push(`${burdenElement} 기운은 ${receiver}에게 상황에 따라 부담으로 커질 수 있습니다.`);
  }
  if (signals.length === 0) {
