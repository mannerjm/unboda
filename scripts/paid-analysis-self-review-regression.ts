...
import { reviewPaidAnalysisDetail } from "../app/lib/paidAnalysisSelfReview";
  ...
  fortuneStructure: {
    ...
    items: [
      ...
      },
      {
        label: "속도",
        value: "단계적 진행",
        interpretation: "서두르기보다 작은 범위에서 검증해야 합니다",
      },
      {
        label: "위험",
        value: "관리 필요",
...
const validReview = reviewPaidAnalysisDetail(baseOutput);

console.log("정상 리포트 Self Review 통과:", validReview.passed);
console.log("정상 리포트 점수:", validReview.overallScore);
...
const invalidOutput: PaidAnalysisDetailOutputV2 = {
  ...
  checklist: [
    baseOutput.actionGuide[1],
    ...baseOutput.checklist.slice(1),
  ],
};

const invalidReview = reviewPaidAnalysisDetail(invalidOutput);

console.log("문제 리포트 Self Review 거부:", !invalidReview.passed);
console.log(
  "추상적 행동 표현 검출:",
  !invalidReview.checks.actionability,
);
console.log(
  "중복 문장 검출:",
  !invalidReview.checks.duplication,
);
console.log("Self Review 피드백 개수:", invalidReview.feedback.length);

if (!validReview.passed) {
  throw new Error(
    `정상 리포트가 Self Review를 통과하지 못했습니다: ${validReview.feedback.join(" | ")}`,
  );
}

if (validReview.overallScore !== 100) {
  throw new Error(
    `정상 리포트 점수가 100점이 아닙니다: ${validReview.overallScore}`,
  );
}

if (invalidReview.passed) {
  throw new Error("문제 리포트가 Self Review에서 거부되지 않았습니다.");
}

if (invalidReview.checks.actionability) {
  throw new Error("추상적인 행동 표현을 검출하지 못했습니다.");
}

if (invalidReview.checks.duplication) {
  throw new Error("중복 문장을 검출하지 못했습니다.");
}

if (invalidReview.feedback.length < 2) {
  throw new Error("문제 리포트의 피드백이 충분히 생성되지 않았습니다.");
}

console.log("paid analysis self review regression passed");