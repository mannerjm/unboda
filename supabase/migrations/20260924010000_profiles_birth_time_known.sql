-- Save explicit unknown/known birth time without modifying existing profile certainty.
-- Historical profiles remain NULL (unverified); never infer certainty from 12:00.
alter table public.profiles add column if not exists birth_time_known boolean;

-- Keep the existing tightly scoped, account/profile limit, no-overwrite guest
-- transfer policy while including the new canonical birth-time-certainty input.
...
begin
  ...
  v_birth_time_known := (v_guest.profile_input ->> 'birthTimeKnown')::boolean;
  ...
  where p.user_id = p_user_id
    ...
    and p.birth_time_known is not distinct from v_birth_time_known
  ...
  if not found then
    ...
    insert into public.profiles (
      user_id, label, relationship_type, birth_date, birth_time, birth_time_known, gender, calendar_type, is_leap_month
    ) values (
      p_user_id, v_label, v_relationship_type, v_birth_date, v_birth_time, v_birth_time_known, v_gender, v_calendar_type, v_is_leap_month
  ...
  v_content_profile := jsonb_build_object(
    ...
    'birthTimeKnown', v_profile.birth_time_known,
