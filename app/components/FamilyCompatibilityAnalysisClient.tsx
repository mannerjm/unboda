          ...
          {RELATIONSHIP_OPTIONS.map((option) => {
            ...
            return (
              <button
                ...
                className={`min-w-[240px] snap-start rounded-[22px] border p-5 text-left transition sm:min-w-0 ${selected ? "border-stone-900 bg-stone-950 text-white shadow-sm" : "border-stone-200 bg-white text-stone-900 hover:border-stone-400"}`}
              >
                <div className="flex items-center justify-between gap-3">
                  <span className="text-base font-bold">{option.title}</span>
                  <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${selected ? "bg-white/10 text-white" : "bg-[#f3eadb] text-stone-700"}`}>
                    {selected ? "선택됨" : "이용 가능"}
                  </span>
                </div>
                <span className={`mt-3 block text-xs leading-6 ${selected ? "text-stone-300" : "text-stone-500"}`}>{option.description}</span>
              </button>
            );
          })}
        </div>

        <p className="mt-4 text-xs leading-6 text-stone-500">가족 관계마다 별도의 해석 기준과 연도판 리포트를 사용하며, 구매한 리포트는 구매 당시 연도판으로 저장됩니다.</p>
      </section>
