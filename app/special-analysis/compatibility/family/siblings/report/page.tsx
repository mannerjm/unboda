...
import { COMPATIBILITY_FAMILY_SIBLING_PRODUCT_ID } from "@/app/lib/specialAnalysisProducts";
...
export default async function FamilySiblingPurchasedReportPage({ searchParams }: Props) {
  ...
  const entitlements = (await listUserEntitlements(user.id))
    .filter((item) => item.profileId === profileId
      && item.resourceId === COMPATIBILITY_FAMILY_SIBLING_PRODUCT_ID
  ...
  if (!entitlement?.analysisEditionKey) {
    redirect("/special-analysis/compatibility/family/parent-child#family-relationship-selector");
  }
