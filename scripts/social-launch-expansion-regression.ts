...
assert(socialGroup?.products.length === 5, `social catalog must expose 5 products, got ${socialGroup?.products.length ?? 0}`);
assert(
  socialGroup?.products.every((product) => expectedSocialCatalogIds.has(product.id))
    && socialGroup.products.every((product) => product.category === "social"),
  "social catalog must expose only the approved five social products",
);
assert(
  expectedSocialCatalogIds.size === new Set(socialGroup?.products.map((product) => product.id)).size,
...
for (const productId of activatedSocialIds) {
  ...
  assert(Boolean(product), `${productId} must exist in the premium registry`);
  ...
  assert(Boolean(config), `${productId} must have a paid topic contract`);
