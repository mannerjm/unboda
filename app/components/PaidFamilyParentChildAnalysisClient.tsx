...
export default function PaidFamilyParentChildAnalysisClient({ myProfileLabel, profileId }: { myProfileLabel: string; profileId: string }) {
      ...
      <div className="bg-[linear-gradient(135deg,#f7f1e6_0%,#fffdf9_60%,#f8f3ea_100%)] px-6 py-8 sm:px-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="text-right">
            <p className="text-sm font-bold text-stone-950">{COMPATIBILITY_FAMILY_PARENT_CHILD_PRODUCT.amount.toLocaleString("ko-KR")}원</p>
            <p className="mt-1 text-xs text-stone-500">{evaluationYear}년판</p>
          </div>
        </div>
        <p className="mt-3 text-sm leading-7 text-stone-600">먼저 이 관계에서 내가 부모인지 자녀인지 선택한 뒤 상대 가족의 정보를 입력해 주세요. 결제 시점의 연도판으로 생성되어 구매한 분석에 그대로 보관됩니다.</p>
      </div>
      ...
      <div className="p-6 sm:p-8">
        ...
        <div className="mt-7 rounded-3xl border border-[#eadfc9] bg-[#faf6ee] p-5">
          ...
          <div className="mt-3 grid gap-2 text-sm leading-7 text-stone-600 sm:grid-cols-2">
            <p>· 정서적 연결과 대화 방식</p><p>· 부모 → 자녀 / 자녀 → 부모의 영향</p><p>· 기대와 독립, 보호와 경계</p><p>· 갈등 뒤 회복과 {evaluationYear}년 관계 흐름</p>
          </div>
        </div>

        <div className="mt-5 rounded-2xl bg-stone-50 px-4 py-4 text-sm leading-7 text-stone-600">입력한 가족의 원본 정보는 결제 전에는 이 브라우저 세션에만 임시 보관합니다. 주문 생성 시 서버에서 계산용 스냅샷으로 변환하며, 구매 리포트는 {evaluationYear}년판으로 고정 저장됩니다.</div>
        {error ? <p className="mt-4 rounded-2xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{error}</p> : null}
        <button type="submit" disabled={!canSubmit} className="mt-6 w-full rounded-2xl bg-stone-950 px-5 py-4 text-sm font-bold text-white transition hover:bg-stone-800 disabled:cursor-not-allowed disabled:bg-stone-300">
          {COMPATIBILITY_FAMILY_PARENT_CHILD_PRODUCT.amount.toLocaleString("ko-KR")}원 · 결제하고 분석하기
        </button>
        <p className="mt-3 text-center text-xs leading-6 text-stone-500">결제 승인 후 개인화 리포트 생성이 시작되고 구매한 분석에 보관됩니다.</p>
      </div>
    </form>
  );
}
