  ...
  return (
    <form onSubmit={submit} className="mx-auto mt-8 max-w-3xl overflow-hidden rounded-[30px] border border-[#dfe3ef] bg-white shadow-[0_18px_50px_rgba(32,38,72,0.08)]">
      <div className="relative overflow-hidden bg-[radial-gradient(circle_at_18%_35%,rgba(112,88,229,0.28),transparent_24%),radial-gradient(circle_at_82%_65%,rgba(83,180,215,0.18),transparent_25%),linear-gradient(145deg,#0b1025_0%,#171a3d_58%,#24204d_100%)] px-6 py-8 text-white sm:px-8 sm:py-9">
        ...
        <p className="mt-2 text-2xl font-bold tracking-[-0.02em] text-white">{myProfileLabel}님의 사주 <span className="font-medium text-[#c9c3ff]">×</span> 가족 사주</p>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <span className="rounded-full border border-white/12 bg-white/[0.07] px-3 py-1.5 text-xs font-semibold text-slate-100">{mode === "siblings" ? "형제·자매 관계" : "기타 가족 관계"}</span>
      ...
      <div className="p-6 sm:p-8">
        {mode === "other_family" ? (
          <div className="grid gap-5 sm:grid-cols-2">
            <label className="sm:col-span-2">
              <span className="text-sm font-semibold text-slate-800">어떤 가족 관계인가요?</span>
              <select
        ...
        <div className="mt-7 grid gap-5 sm:grid-cols-2">
          <label className="sm:col-span-2">
            <span className="text-sm font-semibold text-slate-800">{mode === "siblings" ? "형제·자매" : "상대 가족"} 이름 또는 별칭</span>
            <input value={form.label} onChange={(event) => setForm((current) => ({ ...current, label: event.target.value }))} maxLength={40} required className="mt-2 w-full rounded-2xl border border-[#cfd5e6] bg-white px-4 py-3.5 text-sm outline-none transition focus:border-[#6f5ce7] focus:ring-2 focus:ring-[#e5e1ff]" placeholder={mode === "siblings" ? "예: 언니, 동생, 민지" : "예: 할머니, 조카, 사촌, 형님"} />
          </label>
          ...
          <div>
            <label className="block">
              ...
              <input type="time" value={form.birthTime} disabled={!form.birthTimeKnown} onChange={(event) => setForm((current) => ({ ...current, birthTime: event.target.value }))} required={form.birthTimeKnown} className="mt-2 w-full rounded-2xl border border-[#cfd5e6] bg-white px-4 py-3.5 text-sm outline-none transition disabled:bg-[#eef0f6] disabled:text-slate-400 focus:border-[#6f5ce7]" />
            </label>
            <label className="mt-3 flex items-center gap-2 text-sm text-slate-600"><input type="checkbox" checked={!form.birthTimeKnown} onChange={(event) => setForm((current) => ({ ...current, birthTimeKnown: !event.target.checked }))} />출생시간을 몰라요</label>
          </div>

          <label>
            <span className="text-sm font-semibold text-slate-800">성별</span>
            <select value={form.gender} onChange={(event) => setForm((current) => ({ ...current, gender: event.target.value as FormState["gender"] }))} required className="mt-2 w-full rounded-2xl border border-[#cfd5e6] bg-white px-4 py-3.5 text-sm outline-none focus:border-[#6f5ce7]">
          ...
          <label>
            ...
            <select value={form.calendarType} onChange={(event) => setForm((current) => ({ ...current, calendarType: event.target.value as FormState["calendarType"], isLeapMonth: false, birthDay: "" }))} className="mt-2 w-full rounded-2xl border border-[#cfd5e6] bg-white px-4 py-3.5 text-sm outline-none focus:border-[#6f5ce7]"><option value="양력">양력</option><option value="음력">음력</option></select>
          </label>

          {form.calendarType === "음력" ? <label className="flex items-center gap-2 self-end pb-4 text-sm text-slate-600"><input type="checkbox" checked={form.isLeapMonth} onChange={(event) => setForm((current) => ({ ...current, isLeapMonth: event.target.checked }))} />윤달이에요</label> : null}
        </div>
        ...
        <div className="mt-7 overflow-hidden rounded-3xl border border-[#dfe3ef] bg-[linear-gradient(135deg,#f5f6fc_0%,#f9faff_100%)] shadow-sm">
          <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-end sm:justify-between sm:p-6">
            <div>
              ...
              <p className="mt-2 text-sm font-semibold text-slate-800">{mode === "siblings" ? "형제·자매" : "기타 가족"} 관계 분석을 결제 후 바로 생성합니다.</p>
            ...
            <div className="shrink-0 sm:text-right">
              ...
              <p className="mt-1 text-[22px] font-bold tracking-[-0.02em] text-[#11162d]">{product.amount.toLocaleString("ko-KR")}원</p>
            </div>
          </div>
        </div>

        <button type="submit" disabled={!canSubmit} className="mt-3 inline-flex w-full items-center justify-center rounded-2xl bg-[#6f5ce7] px-5 py-4 text-sm font-bold text-white shadow-[0_12px_28px_rgba(93,76,209,0.2)] transition hover:bg-[#5f4fd2] disabled:cursor-not-allowed disabled:bg-slate-300 disabled:shadow-none">결제하고 궁합 분석하기</button>
