...
function SectionHeader({ number, title, description }: { number: string; title: string; description?: string }) {
  return (
    <div>
      <p className="text-[11px] font-bold tracking-[0.17em] text-[#6f5ce7]">{number}</p>
    ...
    <div className="mt-5 space-y-2.5">
      {points.map((point, index) => (
        <div key={`${point}-${index}`} className="flex gap-3 rounded-2xl border border-[#dce1ef] bg-white px-4 py-3 text-sm leading-6 text-slate-700">
          <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#6f5ce7] text-[10px] font-bold text-white">✓</span>
          <span>{point}</span>
        </div>
      ))}
    </div>
...
function DirectionSection({ directions }: { directions: readonly DirectionCard[] }) {
  return (
    <section className="border-y border-[#d8d3ff] bg-[#f3f1ff] px-6 py-8 sm:px-9 sm:py-10">
      <p className="text-[11px] font-bold tracking-[0.17em] text-[#6f5ce7]">서로에게 미치는 방식</p>
      <h2 className="mt-2 text-2xl font-bold text-[#11162d]">같은 가족이라도 두 방향은 다르게 느껴질 수 있습니다.</h2>
      <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">누가 누구에게 영향을 주는지 방향을 나누어 확인하고, 힘이 되는 부분과 부담으로 느껴질 수 있는 부분을 따로 살펴봅니다.</p>
...
function SiblingReport({ content }: { content: StoredFamilySiblingReport }) {
  ...
  return (
    <article className="mt-8 overflow-hidden rounded-[32px] border border-[#dfe3ef] bg-[#f9faff] shadow-[0_22px_60px_rgba(32,38,72,0.09)]">
      <header className="bg-[linear-gradient(135deg,#f1f0ff_0%,#fbfcff_58%,#eef1fb_100%)] px-6 py-8 sm:px-9 sm:py-10">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full bg-white/80 px-3 py-1.5 text-[11px] font-bold text-slate-700 ring-1 ring-[#dce1ef]">가족 궁합 리포트</span>
        ...
        <p className="mt-5 text-[11px] font-bold tracking-[0.17em] text-slate-400">관계 핵심</p>
        ...
        {!meta.familyMemberBirthTimeKnown ? <p className="mt-5 rounded-2xl bg-white/65 px-4 py-3 text-xs leading-6 text-slate-600 ring-1 ring-[#dce1ef]">상대 형제·자매의 출생시간을 모르는 경우 확인 가능한 생년월일 범위와 구매 연도 흐름을 중심으로 분석합니다.</p> : null}
      </header>

      <DirectionSection directions={[directions.userToSibling, directions.siblingToUser]} />

      <div className="space-y-5 bg-[#f7f8fc] p-6 sm:p-9">
        ...
        {report.currentTiming ? (
          <section className="rounded-[28px] border border-[#d8d3ff] bg-[#f3f1ff] p-6 sm:p-7">
            ...
            <div className="mt-5 rounded-[24px] border border-[#d8d3ff] bg-white/75 p-5 sm:p-6">
              ...
              <p className="mt-4 text-sm leading-7 text-slate-700">{report.currentTiming.summary}</p>
      ...
      <p className="border-t border-[#dce1ef] bg-white px-6 py-5 text-center text-xs leading-6 text-slate-500">이 리포트의 연도 흐름은 구매 당시 {meta.evaluationYear}년판으로 저장됩니다. 가족 궁합은 명리 구조와 해당 연도 흐름을 해석한 참고 콘텐츠이며 한쪽의 성격이나 가족사의 원인을 단정하지 않습니다.</p>
    </article>
  );
}
  ...
  return (
    <article className="mt-8 overflow-hidden rounded-[32px] border border-[#dfe3ef] bg-[#f9faff] shadow-[0_22px_60px_rgba(32,38,72,0.09)]">
      <header className="bg-[linear-gradient(135deg,#f1f0ff_0%,#fbfcff_58%,#eef1fb_100%)] px-6 py-8 sm:px-9 sm:py-10">
        ...
        {!meta.familyMemberBirthTimeKnown ? <p className="mt-5 rounded-2xl bg-white/65 px-4 py-3 text-xs leading-6 text-slate-600 ring-1 ring-[#dce1ef]">상대 가족의 출생시간을 모르는 경우 확인 가능한 생년월일 범위와 구매 연도 흐름을 중심으로 분석합니다.</p> : null}
      </header>
      ...
      <p className="border-t border-[#dce1ef] bg-white px-6 py-5 text-center text-xs leading-6 text-slate-500">이 리포트의 연도 흐름은 구매 당시 {meta.evaluationYear}년판으로 저장됩니다. 가족 궁합은 명리 구조와 해당 연도 흐름을 해석한 참고 콘텐츠이며 가족의 서열이나 한쪽의 잘잘못을 판단하지 않습니다.</p>
    </article>
  );
}
