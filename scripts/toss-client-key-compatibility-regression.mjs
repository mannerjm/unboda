...
assert.ok(checkout.indexOf("getTossCheckoutClientKey()") < orderApiIndex, "runtime public key is fetched before creating an order");
...
assert.doesNotMatch(checkout, /NEXT_PUBLIC_TOSS_CLIENT_KEY/, "checkout must not depend on an inlined public key");
assert.match(config, /TOSS_CLIENT_KEY/, "server reads the client key at runtime");
